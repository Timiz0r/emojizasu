...
Reads:  scripts/emoji-list.txt  (Unicode emoji-test.txt, checked in)
...
EMOJI_LIST = os.path.join(ROOT, "picker", "data", "emoji-list.txt")
...
def _merge_po(path: str, lang: str, keys: list[str], fresh_fn,
    ...
    out = _po_header(lang)
    for key in keys:
        out += existing[key] if key in existing else fresh_fn(key)
...
def _check_categories() -> None:
    with open(EMOJI_LIST, encoding="utf-8") as f:
        groups_in_list = {line[9:].strip() for line in f if line.startswith("# group: ")}
...
def main() -> None:
    with open(EMOJI_LIST, encoding="utf-8") as f:
        emoji_entries = parse_emoji_list(f.read())
    print(f"Loaded {len(emoji_entries)} fully-qualified emoji from emoji-list.txt")
